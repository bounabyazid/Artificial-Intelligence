...
def count_collisions(array):
    ...
    print("Collisions: " + str(len(collissions)))
    return len(collissions)
...
while queue:
    ...
    if checked_pos == 1:
        ...
        possibilities = get_possibilities(popped_array[0][checked_pos[0]][checked_pos[1]])

    for z in possibilities:
        temp_data = copy.deepcopy(popped_array[0])
        temp_data[checked_pos[0]][checked_pos[1]] = z
        collissions = count_collisions(temp_data)
        if temp_data not in queue and temp_data not in extended_list:
            if usr_input == 1:
                queue.append([temp_data, popped_array[1] + 1, collissions])
            elif usr_input == 2:
